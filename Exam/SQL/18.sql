-- Создать хранимую процедуру без параметров, которая перестраивает все индексы 
-- всех таблиц в текущей базе данных. Созданную хранимую процедуру протестировать.
    ...
    WHERE o.type='U' AND o.principal_id IS NULL
        ...
        BEGIN
                ...
                BEGIN
                    ...
                    SELECT @sql ='DBCC DBREINDEX("'+@tablename+'", '''',0)'
                    EXEC ( @sql )
                END
                FETCH NEXT FROM tnames_cursor INTO @tablename
        END
